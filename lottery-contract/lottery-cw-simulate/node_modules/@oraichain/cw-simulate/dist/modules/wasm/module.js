...
exports.lensFromSnapshot = exports.WasmModule = void 0;
...
class WasmModule {
    ...
    registerContractInstance(sender, codeId, label = '', admin = null) {
        return this.store.tx(setter => {
            const contractAddressHash = (0, wasm_util_1.buildContractAddress)(codeId, this.lastInstanceId + 1);
            const contractAddress = (0, encoding_1.toBech32)(this.chain.bech32Prefix, contractAddressHash);
            const contractInfo = {
                codeId,
                creator: sender,
                admin,
    ...
    async instantiateContract(sender, funds, codeId, instantiateMsg, label, admin = null, traces = []) {
    ...
    async migrateContract(sender, newCodeId, contractAddress, migrateMsg, traces = []) {
        return await this.chain.pushBlock(async () => {
            const contract = await this.getContract(contractAddress).init();
    ...
    async executeContract(sender, funds, contractAddress, executeMsg, traces = []) {
        return await this.chain.pushBlock(async () => {
            const contract = await this.getContract(contractAddress).init();
            const logs = [];
            const tracebase = {
    ...
    async handleMsg(sender, msg, traces = []) {
        return this.store.tx(async () => {
            ...
            if ('execute' in wasm) {
                ...
                return await this.executeContract(sender, funds, contract_addr, (0, cosmwasm_stargate_1.fromBinary)(msg), traces);
            }
            if ('instantiate' in wasm) {
                let { code_id, funds, msg, label, admin } = wasm.instantiate;
                return await this.instantiateContract(sender, funds, code_id, (0, cosmwasm_stargate_1.fromBinary)(msg), label, admin, traces);
            }
            if ('migrate' in wasm) {
                let { contract_addr, new_code_id, msg } = wasm.migrate;
                return await this.migrateContract(sender, new_code_id, contract_addr, (0, cosmwasm_stargate_1.fromBinary)(msg), traces);
            }
    ...
    handleQuery(query) {
        ...
        if ('raw' in query) {
            ...
            const value = this.chain.kvIterStorageRegistry === cosmwasm_vm_js_1.BinaryKVIterStorage
                ? // @ts-ignore
                    (0, encoding_1.toBase64)(storage.get((0, encoding_1.fromBase64)(key)))
                : // @ts-ignore
                    storage.get(key);
            if (value === undefined) {
